...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("WebGL renderer creation failed, using fallback:", error);
      // Fallback: create a simple div instead
    ...
    if (!renderer || !renderer.domElement) {
      console.error("Failed to create WebGL renderer or canvas");
      return;
    }
    ...
    try {
      ...
      console.error("Failed to append renderer canvas:", error);
      renderer.dispose();
      return;
    }
    ...
    return () => {
      ...
      if (quad && quad.geometry) (quad.geometry as THREE.BufferGeometry).dispose();
      if (composerRef.current) {
